const ConferenceData = {
  title: "International Conference on Climate Change and Public Health",
  theme: "Current Trends and Research on Climate Change and Public Health",
  date: "July 21-22, 2025",
  location: "Vancouver, Canada",
  deadline: "Abstract Submission Opened",
  buttons: [
    { text: "Register Now", link: "/register" },
    { text: "Abstract Submission", link: "/climate-change-conference/abstract-submission" },
    { text: "Brochure Download", link: "/climate-change-conference/brochure-download" },
    { text: "Tentative Program", link: "pictures/Climate-Change-and-Environmental-Science.pdf", isPDF: true },
  ],
};

export default ConferenceData;
